...
test_that("'also_depends' should operate on ... and their dependencies.", {
    skip_on_cran()
    expect_equal(sum(shelf(tidyverse, lib = tempdir(), cran_repo = "https://cran.r-project.org", quiet = TRUE)), 1)
...
test_that("'quiet' argument should suppress failed-detach message created by 'safe'.", {
    skip_on_cran()
    expect_equal(sum(shelf(dplyr, janitor, lib = tempdir(), cran_repo = "https://cran.r-project.org", quiet = TRUE)), 2)
