...
_Librarian_ helps you install and use packages without interrupting your train of thought by merging CRAN, GitHub, and Bioconductor support into one consistent interface. Installing and loading packages from these different repositories can be done with:
...
_Librarian_ installs packages that aren't on your computer yet, which makes it fast and easy to try out new packages; just add them to the `shelf()` call. And since _Librarian_ handles package installation for you, you won't have to uncomment those package installation lines or wrap them in an `if` to prevent them from installing more than once.
...
You can force packages to reinstall with the `update_all` argument.
...
There is a shortcut for unattaching and reattaching packages (you want to reload the new build of your personal package, for example).
